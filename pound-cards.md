...
Drop `open-commands.js` next to the economy module. Prefix: if `isOpenCommand(token)` skip role + enabled checks. Slash: `slashBuilders` — **never** `setDefaultMemberPermissions`. Re-PUT guild commands so old `.£` permission bits die.
